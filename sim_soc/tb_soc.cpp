...
static bool has_arg(int argc, char **argv, const std::string &needle) {
    for (int i = 1; i < argc; ++i) {
...
int main(int argc, char **argv) {
    ...
    if (wave_enable) {
        ...
        top->trace(tfp, 99);
        tfp->open("tb_soc.vcd");
        std::cout << "[TB-SOC] Waveform enabled: tb_soc.vcd\n";
    }
#endif
    ...
    std::cout << "\n[TB-SOC] cycles      = " << cycles << "\n";
    std::cout << "[TB-SOC] pc          = 0x" << std::hex << top->pc_o << std::dec << "\n";
    std::cout << "[TB-SOC] result_code = 0x" << std::hex << code << std::dec << "\n";
    std::cout << "[TB-SOC] trapped     = " << (trapped ? "yes" : "no") << "\n";
    ...
    if (expect_trap) {
        if (trapped) {
            std::cout << "[TB-SOC] PASS: expected trap observed.\n";
            return 0;
        }
        std::cerr << "[TB-SOC] FAIL: expected trap was not observed.\n";
        return 4;
    }

    if (trapped) {
        std::cerr << "[TB-SOC] FAIL: CPU entered trap state.\n";
        return 2;
    }

    if (timeout) {
        std::cerr << "[TB-SOC] FAIL: timeout before DONE.\n";
        return 3;
    }

    if ((code & 0xFFFF0000u) == 0xCAFE0000u) {
        std::cout << "[TB-SOC] PASS\n";
        return 0;
    }

    std::cerr << "[TB-SOC] FAIL: result code is not a PASS code.\n";
    return 1;
}
